...
class TCLClimate : public climate::Climate, public uart::UARTDevice, public PollingComponent {
 public:
  ...
  bool beep_on = true;
  float temp_offset = -2.2f;  // indoor sensor self-heating bias; tune to taste
  ...
  bool use_external_temp = false;

  void set_temperature_sensor(sensor::Sensor *sens) {
    if (sens == nullptr) return;
    sens->add_on_state_callback([this](float state) {
      this->use_external_temp = true;
      this->current_temperature = state;
      this->publish_state();
    });
  }

  void set_humidity_sensor(sensor::Sensor *sens) {
    if (sens == nullptr) return;
    this->supports_humidity_ = true;
    sens->add_on_state_callback([this](float state) {
